...
def _assign_runnumber(ref_dict, clean_runlist, dict_runs_adjust, main_df, save_dir, run_basename, bids_dict):
    for ind, r in enumerate(clean_runlist): 
        ...
        run_basename = f"{bids_dict['sub']}_{bids_dict['ses']}_{bids_dict['task']}_{clean_run}-{task_type}_recording-ppg-eda_physio.acq"
        run_dir = os.path.join(save_dir, bids_dict['task'], bids_dict['sub'], bids_dict['ses'])
