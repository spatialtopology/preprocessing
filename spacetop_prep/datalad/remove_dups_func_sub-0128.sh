...
error_log="errorlog_BOLD_DUP_sub-0128.txt"
...
dup_files=(
    "./sub-0128/ses-04/func/sub-0128_ses-04_task-social_acq-mb8_run-05_bold__dup-01.json"
)
...
for DUPJSON in "${dup_files[@]}"; do
    ...
    for key in "${!myArray[@]}"; do
        if [[ "$filename" == *"$key"* ]]; then
            ...
            echo -e "\nCASE 1: delete dup-02 DUPS" >> "$error_log"
            ...
            generic_filename=$(echo "$DUPJSON" | sed -E 's/(run-[0-9]+_)[^_]+(__dup-)[0-9]+\..*/\1*\202.*/')
