...
acq_list = glob.glob(os.path.join(main_dir, 'biopac', 'dartmouth', 'b02_sorted', 'sub-*', '**', '*task-social*.acq'), recursive = True)
flaglist = []
...
for acq in sorted(acq_list):
    ...
    try: 
        ...
        # 5) pop run number
        run_list = list(range(len(start_df)))
        run_bool = ((stop_df-start_df)/spacetop_samplingrate)>300
        clean_runlist = list(compress(run_list, run_bool))
        shorter_than_300 = list(compress(run_list, ~run_bool))
        if len(shorter_than_300) > 0:
            flaglist.append(f"runs shorter than 300 sec: {sub} {ses} {shorter_than_300} - run number in python order")
